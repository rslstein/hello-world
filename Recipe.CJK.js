...
void function () {
    ...
    /**
     * Helper function to parse an element and its attributes
     * @param {HTMLElement} element The element to parse
     * @param {Object} accumulator An object to accumulate the results
     */
    function parseElement(element, accumulator) {
        // TODO: ESCAPE STRINGs
        // Parse classes
        element.classList.forEach(_class => {
            let longClassName = `${element.nodeName}_class_${_class}`;
            
            if (accumulator.hasOwnProperty(longClassName)) {
                // If class exists
                accumulator[longClassName] += 1;
            } else {
        ...
        for (var n = 0; n < element.attributes.length; n++) {
            let attributeName = element.attributes[n].name;
            let attributeValue = element.attributes[n].value;
            let longAttributeName = `${element.nodeName}_attrib_${attributeName}_${attributeValue}`;

            if (attributeName !== "class") {
                if (accumulator.hasOwnProperty(longAttributeName)) {
                    // Attribute value already exists
                    accumulator[longAttributeName] += 1;
                } else {
                    // Attribute value does not exist yet
                    accumulator[longAttributeName] = 1;
                }
            }
        }
    ...
    /**
     * Recipe that gets called by the crawler
     * @param {HTMLElement} element The current element
     * @param {object} results Object that stores the result
     */
    function recipe(element, results) {
        // Data template
        let accumulator = {};
        let longElementCount = `${element.nodeName}_count`;

        // Set initial element count
        accumulator[longElementCount] = 0;

        if (results.hasOwnProperty(element.nodeName)) {
            // Fetch cached data
            accumulator = results[element.nodeName];
        }

        // Parse element
        parseElement(element, accumulator);

        // Count this occurance
        accumulator[longElementCount] += 1;

        // Store recent result
        results[element.nodeName] = accumulator;
    }
